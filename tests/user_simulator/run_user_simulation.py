...
def _ensure_unique_target_path(candidate: Path) -> Path:
    """Gera um caminho alternativo caso ``candidate`` já exista."""
    ...
    counter = 1
    while True:
        alt_name = f"{stem}-dup{counter}{suffix}"
        alternative = candidate.with_name(alt_name)
        if not alternative.exists():
            return alternative
        counter += 1


def _collapse_legacy_artifact_dirs(run_dir: Path, artefacts_dir: Path) -> list[str]:
    """Remove diretórios legados de artefatos mantendo apenas ``artefacts``.

    Caso encontremos conteúdo em diretórios antigos (``artfacts`` ou
    ``artifacts``), os arquivos são movidos para o diretório canônico e os
    diretórios antigos são excluídos.
    """

    notes: list[str] = []

    for alias in ("artfacts", "artifacts"):
        alias_dir = run_dir / alias
        ...
        try:
            if alias_dir.is_symlink():
                ...
                notes.append(f"🔗 Atalho legado '{alias}' removido.")
                continue
        except OSError:
            # Se não conseguimos inspecionar o link simbólico, tentamos removê-lo.
            alias_dir.unlink(missing_ok=True)
            notes.append(f"🔗 Atalho legado '{alias}' removido.")
            continue
        ...
        if alias_dir.is_dir():
            moved = 0
            for item in sorted(alias_dir.iterdir()):
                target = _ensure_unique_target_path(artefacts_dir / item.name)
                try:
            ...
            if moved:
                notes.append(
                    f"♻️ {moved} artefato(s) migrados de '{alias}' para 'artefacts'."
                )
            else:
                notes.append(f"♻️ Diretório legado '{alias}' removido.")
        else:
            alias_dir.unlink(missing_ok=True)
            notes.append(f"♻️ Entrada legada '{alias}' removida.")
...
def run_simulation(case_dir: Path, flow: FlowDefinition, *, run_name: str, dry_run: bool) -> Path:
    ...
    for note in _collapse_legacy_artifact_dirs(run_dir, artefacts_dir):
        _emit_cli(note)
