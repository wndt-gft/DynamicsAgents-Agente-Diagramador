...
# Import robusto do metamodel validator (novo caminho: pacote app.metamodel)
try:
    # Caminho correto: estamos em app.tools.generators -> subir dois níveis para app
    from ...metamodel.metamodel_validator import BiZZdesignMetamodelValidator, ValidationSeverity  # type: ignore
except ImportError:
    try:
        # Fallback: adicionar caminho absoluto até a pasta 'metamodel' diretamente sob app
        app_base = os.path.abspath(os.path.join(current_dir, '..', '..'))  # app/tools/generators -> app
        metamodel_dir = os.path.join(app_base, 'metamodel')
        if metamodel_dir not in sys.path:
            sys.path.append(metamodel_dir)
        from metamodel_validator import BiZZdesignMetamodelValidator, ValidationSeverity  # type: ignore
    except ImportError:
        logging.warning("MetamodelValidator não disponível - usando fallback")
        ...
        class BiZZdesignMetamodelValidator:  # type: ignore
            def __init__(self, metamodel_path=None):
                self.metamodel_path = metamodel_path
            def validate_element(self, element_type, element_data):
                return {"is_valid": True, "issues": []}
            def validate_relationship(self, rel_type, source, target):
                return {"is_valid": True, "issues": []}
...
class MetamodelCompliantC4Generator:
    ...
    def __init__(self, metamodel_path: str):
        ...
        # Contexto da geração atual para personalizar metadados da view
        self._current_diagram_type: Optional[str] = None
    ...
    def generate_context_diagram(self, user_story: str, system_name: str) -> str:
        ...
        self._current_diagram_type = 'context'
    ...
    def generate_component_diagram(self, user_story: str, container_name: str) -> str:
        ...
        self._current_diagram_type = 'component'
        self._current_system_name = container_name
    ...
    def generate_container_diagram(self, user_story: str, system_name: str) -> str:
        ...
        self._current_diagram_type = 'container'
        self._current_system_name = system_name
    ...
    def _generate_archimate_xml(self) -> str:
        ...
        default_label = "Vista do Diagrama"
        if self._current_diagram_type == 'context':
            default_label = f"Visão de Contexto - {self._current_system_name or 'Solução'}"
        elif self._current_diagram_type == 'container':
            default_label = f"Visão de Container - {self._current_system_name or 'Solução'}"
        elif self._current_diagram_type == 'component':
            default_label = f"Visão de Componentes - {self._current_system_name or 'Solução'}"
        view_name.text = default_label
